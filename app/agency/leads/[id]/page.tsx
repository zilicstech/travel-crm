...
import { initialLeads, initialAgents, Lead, LeadStatus, LeadCategory, ProposalItem, VisaTracker } from '@/lib/mockData';
...
import {
  ...
  FileText, Link as LinkIcon, ChevronDown, Edit2, Check, Clock, UserCog
} from 'lucide-react';
...
export default function AgencyLeadDetailPage() {
  ...
  const assignedAgent = initialAgents.find(a => a.id === lead.assignedTo);
  ...
  const assignAgent = (agentId: string) => {
    setLead(l => l ? { ...l, assignedTo: agentId } : l);
    setShowAssignDropdown(false);
  };

  const addNote = () => {
    if (!noteText.trim()) return;
    const author = 'John Davis'; // agency owner
    const note = { id: `N${Date.now()}`, text: noteText.trim(), timestamp: new Date().toISOString(), author };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => router.push('/agency/leads')} className="flex items-center text-xs text-blue-600 hover:text-blue-800 font-semibold transition-colors">
          <ArrowLeft className="w-3.5 h-3.5 mr-1.5" /> Leads
        </button>
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Assigned Agent</p>
                {assignedAgent ? (
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-sm font-bold text-slate-900">{assignedAgent.name}</span>
                    <span className="text-xs text-slate-400">· {assignedAgent.department}</span>
                  </div>
                ) : (
                  <p className="text-sm font-medium text-slate-400 italic mt-0.5">Unassigned</p>
                )}
              </div>
            </div>
            ...
            <div className="relative">
              ...
              {showAssignDropdown && (
                <div className="absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-20 w-56 py-1">
                  <p className="px-4 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">Select Agent</p>
                  {initialAgents.map(agent => (
                    <button
                      key={agent.id}
                      onClick={() => assignAgent(agent.id)}
                      className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-slate-50 transition-colors ${lead.assignedTo === agent.id ? 'text-blue-600' : 'text-slate-700'}`}
                    >
                      <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-[9px] flex-shrink-0">
                        {agent.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div className="text-left">
                        <p className="font-semibold text-xs">{agent.name}</p>
                        <p className="text-[10px] text-slate-400">{agent.department}</p>
                      </div>
                      {lead.assignedTo === agent.id && <Check className="w-3.5 h-3.5 ml-auto" />}
                    </button>
                  ))}
                </div>
              )}
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">Source: {lead.source}</p>
            </div>
          </div>
            ...
            {visaSteps.map((step, idx) => {
              ...
              return (
                <button key={step.key} onClick={() => toggleVisaStep(step.key)}
                  className={`w-full flex items-center gap-4 p-4 rounded-xl border transition-all text-left ${done ? 'bg-green-50 border-green-200' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${done ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                  ...
                  {done ? <CheckSquare className="w-5 h-5 text-green-500 flex-shrink-0" /> : <Square className="w-5 h-5 text-slate-300 flex-shrink-0" />}
                </button>
              );
            })}
      ...
      <Card>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex gap-3 mb-6">
            <div className="w-8 h-8 flex-shrink-0 rounded-full bg-slate-200 flex items-center justify-center text-slate-700 font-bold text-xs">JD</div>
