...
const CURRENT_AGENT_ID = '1'; // Mock logged in agent

export default function AgentsClientsPage() {
  const [clients, setClients] = useState<Client[]>(initialClients.filter(c => c.owningAgentId === CURRENT_AGENT_ID));
  ...
  const handleAddClient = () => {
    ...
    const client: Client = {
      ...
      owningAgentId: CURRENT_AGENT_ID, // Auto-assign to current agent
      mainContactId,
      isActive: true,
                ...
                return (
                  <div key={client.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex items-center gap-6 text-right">
                      ...
                      <Link href={`/agents/clients/${client.id}`}>
                        <Button
                          variant="ghost"
                          size="sm"
